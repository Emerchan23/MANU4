...
async function automateAppointmentCreation() {
    ...
    try {
        ...
        // Função para selecionar em dropdown/select customizado
        async function selectFromDropdown(buttonSelector, optionText) {
            try {
                // Clicar no botão do dropdown
                await page.waitForSelector(buttonSelector, { timeout: 10000 });
                await page.click(buttonSelector);
                await page.waitForTimeout(1000);
                
                // Procurar e clicar na opção
                const optionSelector = `[role="option"]:has-text("${optionText}"), .option:has-text("${optionText}"), li:has-text("${optionText}")`;
                await page.waitForSelector(optionSelector, { timeout: 5000 });
                await page.click(optionSelector);
                
                console.log(`✅ Selecionado do dropdown: ${optionText}`);
                await page.waitForTimeout(500);
            } catch (error) {
                console.log(`⚠️ Erro ao selecionar ${optionText}: ${error.message}`);
            }
        }
