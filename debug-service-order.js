...
async function debugServiceOrder() {
  let connection
  ...
  try {
    ...
    try {
      ...
      console.log('❌ Tabela empresas não existe, tentando companies...')
      try {
    ...
    let companies = []
    try {
      const [companiesResult] = await connection.execute('SELECT id, nome FROM empresas LIMIT 5')
      companies = companiesResult
    } catch (error) {
      try {
        const [companiesResult] = await connection.execute('SELECT id, name FROM companies LIMIT 5')
        companies = companiesResult
      } catch (error2) {
        companies = []
      }
